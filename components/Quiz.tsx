  ...
  if (showResult) {
    return (
      <div className="fixed inset-0 bg-white z-50 flex flex-col items-center justify-center p-6 text-center">
        ...
        <button 
          onClick={onClose}
          className="bg-green-600 text-white px-12 py-4 rounded-2xl font-bold text-lg hover:bg-green-700 transition-all shadow-xl"
        >
          Hasierara Itzuli
        </button>
      </div>
    ...
    <div className="fixed inset-0 bg-gray-50 z-50 flex flex-col">
      <header className="p-4 bg-white border-b border-gray-200 flex justify-between items-center">
        <div className="flex flex-col">
          <span className="text-xs font-bold text-gray-400 uppercase">Progresua</span>
          <div className="flex gap-1 mt-1">
            {questions.map((_, i) => (
              <div 
                key={i} 
                className={`h-1.5 w-6 rounded-full transition-colors ${i < currentQuestion ? 'bg-green-500' : i === currentQuestion ? 'bg-green-400 animate-pulse' : 'bg-gray-200'}`}
              ></div>
            ))}
          </div>
        </div>
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-6 max-w-2xl mx-auto w-full">
        <div className="mb-12 text-center">
          <span className="inline-block px-3 py-1 bg-green-100 text-green-700 rounded-full text-xs font-bold uppercase mb-4">Zer esan nahi du?</span>
          <h2 className="text-3xl md:text-4xl font-bold text-gray-900 heading-font leading-tight">"{idiom.eu}"</h2>
        </div>
        ...
        <div className="w-full space-y-4">
          {options.map((option, idx) => (
            <button
              ...
              disabled={!!answered}
              className={`w-full p-6 text-left rounded-2xl border-2 transition-all font-semibold text-lg ${
                answered 
                  ? option === idiom.es 
                  ...
                  : 'bg-white border-gray-200 hover:border-green-400 hover:shadow-md text-gray-700'
              }`}
            >
      ...
      <footer className="p-6 bg-white border-t border-gray-200">
        <button 
          onClick={handleNext}
          disabled={!answered}
          className={`w-full max-w-2xl mx-auto block py-4 rounded-2xl font-bold text-lg transition-all shadow-lg ${answered ? 'bg-gray-900 text-white hover:bg-black' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}
        >
